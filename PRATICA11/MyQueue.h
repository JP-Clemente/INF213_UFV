...
void MyQueue<T>::push(const T &el)
{
	...
	else
	{
		if (full())
		{
			...
			if (first != 0)
			{ //se first ==0 --> a fila comecava no inicio da lista e terminava no final --> nao precisamos fazer nada
				...
				for (int i = 0; i < first; i++)
					vec[(tamanhoAntigoLista + i)] = vec[i];
...
{
	if (first == last)
	{ // ha apenas um elemento? temos que marcar first como sendo -1 para indicar fila vazia...
		first = last = -1;
	}
	else
